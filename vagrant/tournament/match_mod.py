class MatchStore():
    ...
    def allMatches(self, tournament):
        rows = self.db.fetchAll(
            "SELECT p1, score1, p2, score2 FROM MATCH WHERE TOURNAMENT=%s AND ROUND=%s;", (tournament.id, tournament.round,))
        matches = []
        for row in rows:
    ...
    def getByPlayers(self, tournament, player1, player2):
        ...
        rows = self.db.fetchAll(
            query, (tournament.id, tournament.round, player1, player2,))
        if len(rows) < 1:
            rows = self.db.fetchAll(
                query, (tournament.id, tournament.round, player2, player1,))
            if len(rows) < 1:
                return None
    ...
    def createMatch(self, tournament, player1, player2):
        ...
        self.db.execute("INSERT INTO MATCH VALUES (%s,%s,%s,null,%s,null);",
                        (match.tournament, match.round, match.p1, match.p2,))
        return match
